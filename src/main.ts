...
const PARTICLE_LIFE = 2; // in second
const PARTICLE_MAX_SPEED = 12;
...
function generateParticles(pos: Vector2) {
...
function drawParticles(ctx: CanvasRenderingContext2D): void {
	for (let i = 0; i < particlesArray.length; ++i) {
		for (let j = 0; j < PARTICLE_QUANTITY; ++j) {
			...
			if (particle)
				drawRectangle(ctx, particle.rect.pos, particle.rect.width, particle.rect.height, particle.rect.color);
		}
	}
}
...
function updateParticles(dt: number) {
	for (let i = 0; i < particlesArray.length; ++i) {
		for (let j = 0; j < PARTICLE_QUANTITY; ++j) {
			const particle = particlesArray[i]?.[j];
			if (particle) {
				particle.rect.pos.x += particle.vx * dt;
				particle.rect.pos.y += particle.vy * dt;
			}
		}
	}
}
...
function playGame(ctx: CanvasRenderingContext2D, dt: number) {
	if (!paused) {
		if (ballPos.x + ballRadius >= windowWidth || ballPos.x - ballRadius <= 0) {
			ballVelocity.x *= -1;
			generateParticles({x: (windowWidth - PARTICLE_SIZE) / 2, y: (windowHeight - PARTICLE_SIZE) / 2});
		}
